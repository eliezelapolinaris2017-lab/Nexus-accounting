const navItems = [
  ...
  ['taxes','Impuestos'],['financials','Estados Financieros'],['settings','Configuración']
];
...
function totals(){
  ...
  return {assets,liabilities,equity,income,expense,net:income-expense, ar:balanceByAccount('1300'), ap:balanceByAccount('2100'), bank:balanceByAccount('1200')};
}
...
function render(page){ active=page; document.getElementById('pageTitle').textContent=navItems.find(n=>n[0]===page)?.[1]||'Dashboard'; renderNav(); const map={dashboard,chart,journal,ledger,invoices,ar,ap,banks,taxes,financials,settings}; document.getElementById('content').innerHTML = map[page](); }
...
function dashboard(){
  ...
  return `<div class="grid kpi-grid">
    ...
    ${kpi('Ingresos',t.income,'Mes actual','positive')}${kpi('Gastos',t.expense,'Mes actual','negative')}${kpi('Cuentas por Cobrar',t.ar,'Facturas pendientes')}${kpi('Banco',t.bank,'Disponible')}
  </div>
  <div class="grid two" style="margin-top:16px">
    ...
    <div class="card"><h3>Nexus Score Contable</h3><div class="kpi"><div class="value ${score>=80?'positive':score>=60?'warning':'negative'}">${score}/100</div><div class="sub">Indicador básico por liquidez, utilidad y cobros.</div></div><hr><div class="quick"><button onclick="openModal('invoice')">Nueva factura</button><button onclick="openModal('expense')">Registrar gasto</button><button onclick="render('financials')">Estados</button></div></div>
  </div>`;
}
...
function banks(){ return `<div class="card"><h3>Bancos</h3><div class="table-wrap"><table class="table"><thead><tr><th>Banco</th><th>Cuenta contable</th><th>Saldo</th></tr></thead><tbody>${db.bankAccounts.map(b=>`<tr><td>${b.name}</td><td>${account(b.account).name}</td><td>${money(balanceByAccount(b.account))}</td></tr>`).join('')}</tbody></table></div></div>`; }
